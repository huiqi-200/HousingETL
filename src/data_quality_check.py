...
def data_validation(df: pl.DataFrame) -> tuple[pl.DataFrame, pl.DataFrame]:
    ...
    # 2. Filter for dates required in test
    ...
    unqualified_df = pl.concat(
        [non_numeric_df, missing_from_expected_cat_df, df_duplicates_subset]
    )
    logger.info(
...
def filter_month_range(df: pl.DataFrame) -> pl.DataFrame:
    """Return only rows where ``month`` lies in value specified in data_quality_rules.json under filters.month

    The column is assumed to be a ``pl.Date`` type; the start/end boundaries
    are inclusive. The function is used by :func:`data_validation`.
    """
    start =datetime.strptime(rules["filters"]["month"]["min_month"], "%Y-%m")
    end = datetime.strptime(rules["filters"]["month"]["max_month"], "%Y-%m")
    filtered = df.filter(pl.col("month").is_between(start, end))
    logger.info(f"Filtered rows by month range: kept {filtered.height} of {df.height}")
    return filtered
...
def get_column_resale_identifier(df: pl.DataFrame) -> pl.Series:
    ...
    agg_df = df_new_column.group_by(["year", "month", "X", "X3"]).agg(
